...
const singletons = {
  home: {
    ...
    fields: [
      ...
      { group: "Registration" },
      { name: "site.registrationUrl", label: "Registration link (the “Registration” button on the homepage)", type: "url", hint: "Paste your Google Form link. Leave empty to hide the button." },
      { group: "Site-wide" },
  ...
  labels: {
    ...
    fields: [
      ...
      { name: "nav.registration", label: "Registration button", type: "i18n" },
      { group: "Home page" },
...
const collections = {
  ...
  participants: {
    ...
    key: "participants",
    summary: (it) =>
      `${[it.name, it.surname].filter(Boolean).join(" ") || "(no name)"}${it.affiliation ? " — " + it.affiliation : ""}`,
    fields: [
      { name: "name", label: "First name(s)", type: "text", required: true },
      { name: "surname", label: "Surname (used for alphabetical order)", type: "text" },
      { name: "affiliation", label: "Affiliation", type: "text" },
  ...
  abstracts: {
    ...
    fields: [
      ...
      { name: "pdf", label: "Abstract PDF (optional)", type: "file" },
    ],
  },
