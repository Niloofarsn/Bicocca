...
// Apply submitted form data (body + uploaded files) onto the data object.
function applyForm(fields, body, files, obj) {
  const fileByName = {};
  (files || []).forEach((f) => { fileByName[f.fieldname] = f; });
  ...
  for (const field of fields) {
    ...
    switch (field.type) {
      ...
      case "image": {
        ...
        if (f) setPath(obj, name, "/uploads/" + f.filename);
